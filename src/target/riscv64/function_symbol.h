...
static inline bool minic_riscv64_function_symbol_from_function(
    const MinicFunction *function, MinicRiscv64FunctionSymbol *symbol) {
    const char *symbol_name;
...
static inline bool minic_riscv64_emit_function_symbol_begin(
    FILE *file, const MinicRiscv64FunctionSymbol *symbol) {
    ...
    if (!symbol->is_internal) {
        if (fprintf(file,
                    symbol->is_weak ? ".weak %s\n" : ".globl %s\n",
                    symbol->symbol_name) < 0) {
            return false;
        }
        if (symbol->visibility != MINIC_SYMBOL_VISIBILITY_DEFAULT) {
            visibility_directive =
                minic_riscv64_function_visibility_directive(symbol->visibility);
...
static inline bool minic_riscv64_emit_function_symbol_end(
    FILE *file, const MinicRiscv64FunctionSymbol *symbol) {
