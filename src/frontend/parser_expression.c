...
static bool normalize_float_binary_operands(MinicParser *parser,
    ...
    /* For float/integer arithmetic the usual arithmetic conversions convert
       the integer operand to binary32, not binary64. Do that first; the
       existing float/float path below then keeps arithmetic in binary32 and
       widens only comparisons where exact widening preserves the result. */
    if ((minic_type_is_float(left->type) && minic_type_is_integer(right->type)) ||
        (minic_type_is_integer(left->type) && minic_type_is_float(right->type))) {
        ...
        if (minic_type_is_integer(right->type)) {
            (void)memset(&conversion, 0, sizeof(conversion));
            conversion.kind = MINIC_EXPRESSION_CAST;
            conversion.span = right->span;
            conversion.type = minic_type_float();
    ...
    /* Comparing binary32 values after exact widening to binary64 preserves all
       ...
       conversion to double. Float/integer arithmetic has already converted the
       integer operand to float above so its rounding semantics stay binary32. */
    eligible =
