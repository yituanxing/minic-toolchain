#ifndef MINIC_FRONTEND_TOKEN_H
#define MINIC_FRONTEND_TOKEN_H

#include <stddef.h>

typedef struct MinicSourcePosition {
    size_t offset;
    size_t line;
    size_t column;
} MinicSourcePosition;

typedef struct MinicSourceSpan {
    MinicSourcePosition begin;
    MinicSourcePosition end;
} MinicSourceSpan;

typedef enum MinicTokenKind {
    MINIC_TOKEN_INVALID = 0,
    MINIC_TOKEN_EOF,
    MINIC_TOKEN_IDENTIFIER,
    MINIC_TOKEN_INTEGER_CONSTANT,
    MINIC_TOKEN_CHARACTER_CONSTANT,
    MINIC_TOKEN_FLOATING_CONSTANT,
    MINIC_TOKEN_STRING_LITERAL,
    MINIC_TOKEN_KW_CHAR,
    MINIC_TOKEN_KW_DOUBLE,
    MINIC_TOKEN_KW_FLOAT,
    MINIC_TOKEN_KW_INT,
    MINIC_TOKEN_KW_LONG,
    MINIC_TOKEN_KW_SHORT,
    MINIC_TOKEN_KW_SIGNED,
    MINIC_TOKEN_KW_UNSIGNED,
    MINIC_TOKEN_KW_VOID,
    MINIC_TOKEN_KW_STRUCT,
    MINIC_TOKEN_KW_UNION,
    MINIC_TOKEN_KW_ENUM,
    MINIC_TOKEN_KW_CONST,
    MINIC_TOKEN_KW_TYPEDEF,
    MINIC_TOKEN_KW_EXTERN,
    MINIC_TOKEN_KW_STATIC,
    MINIC_TOKEN_KW_SIZEOF,
    MINIC_TOKEN_KW_RETURN,
    MINIC_TOKEN_KW_IF,
    MINIC_TOKEN_KW_ELSE,
    MINIC_TOKEN_KW_WHILE,
    MINIC_TOKEN_KW_DO,
    MINIC_TOKEN_KW_FOR,
    MINIC_TOKEN_KW_SWITCH,
    MINIC_TOKEN_KW_CASE,
    MINIC_TOKEN_KW_DEFAULT,
    MINIC_TOKEN_KW_BREAK,
    MINIC_TOKEN_KW_CONTINUE,
    MINIC_TOKEN_LPAREN,
    MINIC_TOKEN_RPAREN,
    MINIC_TOKEN_LBRACE,
    MINIC_TOKEN_RBRACE,
    MINIC_TOKEN_SEMICOLON,
    MINIC_TOKEN_COMMA,
    MINIC_TOKEN_QUESTION,
    MINIC_TOKEN_COLON,
    MINIC_TOKEN_DOT,
    MINIC_TOKEN_ELLIPSIS,
    MINIC_TOKEN_PLUS,
    MINIC_TOKEN_PLUS_PLUS,
    MINIC_TOKEN_PLUS_EQUAL,
    MINIC_TOKEN_MINUS,
    MINIC_TOKEN_MINUS_MINUS,
    MINIC_TOKEN_MINUS_EQUAL,
    MINIC_TOKEN_ARROW,
    MINIC_TOKEN_STAR,
    MINIC_TOKEN_STAR_EQUAL,
    MINIC_TOKEN_AMPERSAND,
    MINIC_TOKEN_AMPERSAND_EQUAL,
    MINIC_TOKEN_AMPERSAND_AMPERSAND,
    MINIC_TOKEN_PIPE,
    MINIC_TOKEN_PIPE_EQUAL,
    MINIC_TOKEN_PIPE_PIPE,
    MINIC_TOKEN_CARET,
    MINIC_TOKEN_CARET_EQUAL,
    MINIC_TOKEN_SLASH,
    MINIC_TOKEN_PERCENT,
    MINIC_TOKEN_EQUAL,
    MINIC_TOKEN_EQUAL_EQUAL,
    MINIC_TOKEN_BANG,
    MINIC_TOKEN_BANG_EQUAL,
    MINIC_TOKEN_TILDE,
    MINIC_TOKEN_LESS,
    MINIC_TOKEN_LESS_LESS,
    MINIC_TOKEN_LESS_EQUAL,
    MINIC_TOKEN_GREATER,
    MINIC_TOKEN_GREATER_GREATER,
    MINIC_TOKEN_GREATER_GREATER_EQUAL,
    MINIC_TOKEN_GREATER_EQUAL,
    MINIC_TOKEN_LBRACKET,
    MINIC_TOKEN_RBRACKET,
    MINIC_TOKEN_KIND_COUNT
} MinicTokenKind;

typedef struct MinicToken {
    MinicTokenKind kind;
    MinicSourceSpan span;
} MinicToken;

const char *minic_token_kind_name(MinicTokenKind kind);

#endif