...
static bool remap_expression_id(
...
{
    if (mapping == NULL || new_id == NULL ||
        old_id >= current_old_index ||
        old_id >= old_expression_count ||
...
static bool remap_non_cast_expression(
...
static bool append_normalized_cast(
    ...
    MinicExpressionId *normalized_id)
{
    ...
    if (rewritten == NULL || cast_expression == NULL ||
        normalized_id == NULL ||
...
static bool remap_program_expression_id(
    const MinicExpressionId *mapping,
    size_t old_expression_count,
...
bool minic_c0_program_normalize_casts(MinicC0Program *program)
{
    ...
    if (program == NULL ||
        (program->expression_count != 0U && program->expressions == NULL) ||
    ...
    if (old_expression_count != 0U) {
        mapping = (MinicExpressionId *)malloc(
            old_expression_count * sizeof(*mapping));
        if (mapping == NULL) {
            return false;
        }
        for (expression_index = 0U;
             expression_index < old_expression_count;
             ++expression_index) {
    ...
    if (program->statement_count != 0U) {
        remapped_statements = (MinicStatement *)malloc(
    ...
    success = true;
    for (expression_index = 0U;
         success && expression_index < old_expression_count;
         ++expression_index) {
        ...
        if (old_expression->kind == MINIC_EXPRESSION_CAST) {
            ...
            success = remap_expression_id(
                          mapping,
                          old_expression_count,
                          expression_index,
                          old_expression->value.unary.operand,
                          &mapped_operand) &&
                      append_normalized_cast(
                          &rewritten,
                          old_expression,
                          mapped_operand,
                          &new_id);
            ...
            success = remap_non_cast_expression(
                          ...
                          old_expression_count,
                          expression_index) &&
                      minic_c0_program_add_expression(
                          &rewritten,
                          &copied_expression,
                          &new_id);
        }
    ...
    for (statement_index = 0U;
         success && statement_index < program->statement_count;
         ++statement_index) {
        MinicStatement *statement;

        statement = &remapped_statements[statement_index];
        success = remap_program_expression_id(
                      mapping,
                      old_expression_count,
