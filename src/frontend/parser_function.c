...
static bool function_attribute_list_has_used(const MinicParsedAttributeList *attributes) {
    ...
    for (index = 0U; index < attributes->count; ++index) {
        ...
        if (descriptor != NULL && descriptor->kind == MINIC_ATTRIBUTE_USED) {
            return true;
        }
    }
    return false;
...
static bool consume_function_attribute(MinicParser *parser,
    ...
    if (descriptor->kind == MINIC_ATTRIBUTE_SECTION) {
        if (context->section_name == NULL || context->section_name_length == NULL ||
            ...
            !minic_parser_apply_section_attribute(parser,
                                                  attribute,
    ...
    if (descriptor->kind == MINIC_ATTRIBUTE_USED) {
        if (context->force_emit != NULL) {
            *context->force_emit = true;
        }
        return true;
    }
...
static bool parameter_array_modifier_is_restrict(const MinicParser *parser) {
    return function_identifier_is(parser, "restrict") ||
           function_identifier_is(parser, "__restrict") ||
...
static bool adjust_array_parameter_type(MinicParser *parser,
    ...
    if (parser->current.kind == MINIC_TOKEN_LBRACKET) {
        bool saw_static;
        bool has_outer_bound;
        size_t ignored_outer_bound;

        saw_static = false;
        has_outer_bound = false;
        if (!minic_parser_advance(parser)) {
            return false;
        }
        while (parser->current.kind == MINIC_TOKEN_KW_STATIC ||
               parser->current.kind == MINIC_TOKEN_KW_CONST ||
               parser->current.kind == MINIC_TOKEN_KW_VOLATILE ||
               parameter_array_modifier_is_restrict(parser)) {
        ...
        if (parser->current.kind == MINIC_TOKEN_RBRACKET) {
            if (saw_static) {
                minic_parser_error(parser, "static array parameter requires a bound");
                return false;
            }
            if (!minic_parser_advance(parser)) {
                return false;
            ...
            if (!minic_parser_parse_fixed_array_bound(parser, &ignored_outer_bound)) {
                return false;
            }
            has_outer_bound = true;
        }
        (void)has_outer_bound;
        (void)ignored_outer_bound;

        pointee_type = *parameter_type;
        if (parser->current.kind == MINIC_TOKEN_LBRACKET) {
            if (!minic_parser_parse_array_declarator_suffix(
                    parser, *parameter_type, false, &declared_array_type, &is_array) ||
                !is_array || !minic_type_is_array(declared_array_type)) {
                if (parser->diagnostic != NULL && parser->diagnostic->message[0] == '\0') {
                    minic_parser_error(parser, "cannot parse nested array parameter declarator");
                }
                return false;
            }
            pointee_type = declared_array_type;
        }
...
static bool record_external_tentative_object(MinicParser *parser,
                                             MinicType object_type,
                                             MinicSourceSpan name_span,
                                             const char *section_name,
                                             size_t section_name_length,
...
static bool
parse_external_tentative_declaration_list_after_head(MinicParser *parser,
                                                     MinicType base_type,
    ...
    while (parser->current.kind == MINIC_TOKEN_COMMA) {
        ...
        if (!minic_parser_parse_pointer_declarator(parser, object_type, &object_type) ||
        ...
        has_section = false;
        has_visibility = first_has_visibility;
        if (!minic_parser_parse_gnu_object_attribute_lists_with_visibility(parser,
                                                                           section_name,
                                                                           sizeof(section_name),
                                                                           &section_name_length,
                                                                           &has_section,
        ...
        if (parser->current.kind == MINIC_TOKEN_EQUAL) {
            minic_parser_error(parser,
                               "mixed initialized external declarator lists are not supported yet");
            return false;
        }
        if (!record_external_tentative_object(parser,
                                              object_type,
                                              name_span,
                                              section_name,
                                              section_name_length,
...
static bool parse_inferred_external_record_array_definition(MinicParser *parser,
    ...
    if (record == NULL || !record->is_complete) {
        minic_parser_error(
            parser, "inferred external record array requires a complete record element type");
        return false;
    }
    if (!minic_parser_advance(parser) ||
        !minic_parser_expect(
            parser, MINIC_TOKEN_RBRACKET, "expected ']' in inferred external record array") ||
...
static bool finish_function_declaration_entity(MinicParser *parser,
    ...
    if (force_emit &&
        !minic_c0_program_set_function_force_emit(parser->program, function_id, true)) {
        minic_parser_error(parser, "cannot persist GNU used function metadata");
        return false;
    }
...
static bool parse_function(MinicParser *parser, bool is_internal) {
    ...
    force_emit = function_attribute_list_has_used(&deferred_attributes) ||
                 function_attribute_list_has_used(&declarator_attributes);
    ...
    if (!is_function_pointer_object && minic_type_is_function(return_type)) {
        ...
        for (;;) {
            ...
            if (function_id == MINIC_FUNCTION_INVALID ||
                (entity_force_emit &&
                 !minic_c0_program_set_function_force_emit(
                     parser->program, function_id, true))) {
                minic_parser_error(parser, "cannot persist GNU used function metadata");
                return false;
            }
            if (parser->current.kind == MINIC_TOKEN_SEMICOLON) {
