...
static bool minic_lexer_scan_integer_suffix(MinicLexer *lexer,
    ...
    while (minic_lexer_peek(lexer) == 'l' || minic_lexer_peek(lexer) == 'L' ||
        ...
        if (suffix == 'l' || suffix == 'L') {
            if (saw_long) {
                minic_lexer_set_message(lexer, diagnostic, begin, "long long constants are not supported");
                return false;
            }
            saw_long = true;
        } else {
            if (saw_unsigned) {
                minic_lexer_set_message(lexer, diagnostic, begin, "duplicate unsigned integer suffix");
                return false;
            }
            saw_unsigned = true;
        }
        suffix_count += 1U;
        if (suffix_count > 2U) {
            minic_lexer_set_message(lexer, diagnostic, begin, "unsupported integer constant suffix");
            return false;
        }
        minic_lexer_advance(lexer);
    }
