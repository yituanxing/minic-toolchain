...
static bool append_parameter_ingress(MinicCoreFunction *function,
    ...
    if (!minic_core_function_append_value_instruction(
            function, block_id, &instruction, address_id)) {
        return false;
    }
...
static bool
set_return(MinicCoreFunction *function, MinicCoreBlockId block_id, MinicCoreValueId value_id) {
...
static bool build_ninth_parameter(MinicCoreFunction *function) {
    ...
    for (index = 0U; index < 9U; ++index) {
        if (!append_parameter_ingress(
                function, entry, index, parameters[index], &addresses[index])) {
            return false;
        }
    }
