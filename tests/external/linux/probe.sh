...
if test ! -s "$input"; then
    printf '%s\n' 'LINUX_DEBUG_ERROR missing init/main.i' >&2
    exit 1
fi

python3 "$root/tools/dev/pr245-rv64-emitter-debug.py"
make -j4 -C "$root" MODE=release BUILD_DIR="$root/build/pr245-rv64-debug" >/dev/null
minic="$root/build/pr245-rv64-debug/bin/minic"
...
printf '%s\n' "PR245_RV64_DEBUG_STATUS=$status" >&2
