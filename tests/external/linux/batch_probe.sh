...
for line in plan_path.read_text(errors="replace").splitlines():
    ...
    if not sources:
        # GCC is also used for .S and binary/special targets. They are configured
        # objects, but they are not C translation units and must not enter this gate.
        continue
