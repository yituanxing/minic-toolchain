    ...
    for instruction in "$@"; do
        case "$instruction" in
            "mv s0, sp"|"sd a0, 0(sp)"|"ld t0, 0(sp)"|"addi a0, s0, "*)
                ...
                ;;
            "j .Lmain_return")
                # Core uses a far PC-relative transfer so return edges remain
                # valid even when a bootstrap-generated function exceeds JAL's
                # signed 21-bit displacement. Accept the historical short jump
...
MINIC="$minic" HOST_CC="$host_cc" BUILD_DIR="${BUILD_DIR:-"$root/build/debug"}" \
 ...
 \
                    "$work/$name.s" >/dev/null; then
                    grep -E '^[[:space:]]+lla[[:space:]]+t6,[[:space:]]*\.Lmain_core_bb[0-9]+
            "beqz a0, .Lif_"*)
...
MINIC="$minic" HOST_CC="$host_cc" BUILD_DIR="${BUILD_DIR:-"$root/build/debug"}" \
 ...
 \
                        "$work/$name.s" >/dev/null
                    test "$(grep -E -c '^[[:space:]]+lla[[:space:]]+t6,[[:space:]]*\.Lmain_core_bb[0-9]+
            "li a0, "*)
                immediate=${instruction#"li a0, "}
                grep -E "^[[:space:]]+li[[:space:]]+[^,]+,[[:space:]]*$immediate$" \
                    "$work/$name.s" >/dev/null
