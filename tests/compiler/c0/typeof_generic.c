...
extern __attribute__((
    section(".probe.typeof.incomplete"))) __typeof__(struct TypeofPending) typeof_pending_object;
...
unsigned long generic_selected_value(unsigned long value) {
    return _Generic(value, int: 3UL, unsigned long: 7UL, default: 9UL);
}
...
unsigned long generic_controlling_is_unevaluated(void) {
    return _Generic(generic_side_effect(), int: 11UL, default: 13UL);
}
...
unsigned long typeof_generic_pointer_size(const void *addr) {
    return sizeof(const volatile typeof(_Generic((*(unsigned long *)addr),
        char: (char)0,
...
unsigned long typeof_array_type_name_size(void) {
    return sizeof(__typeof__(unsigned long[0x1000 / sizeof(long)]));
}
