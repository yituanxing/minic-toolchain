...
if declaration_anchor not in source:
    raise SystemExit("extended caller ABI declaration anchor missing")
...
if first_start < 0:
    raise SystemExit("extended caller ABI count-pass start anchor missing")
if source.find(first_start_anchor, first_start + 1) >= 0:
    raise SystemExit("extended caller ABI count-pass start anchor is not unique")
first_end = source.find(first_end_anchor, first_start)
if first_end < 0:
    raise SystemExit("extended caller ABI count-pass end anchor missing")
...
first_replacement = r'''        {
            ...
            for (argument_index = 0U; argument_index < argument_count; ++argument_index) {
                ...
                if (!minic_riscv64_abi_place_argument(program,
                    ...
                    argument_locations[argument_index].value.kind != abi_values[argument_index].kind ||
                    argument_locations[argument_index].value.storage_size !=
                        abi_values[argument_index].storage_size) {
                    return false;
                }
            }
...
if second_start < 0:
    raise SystemExit("extended caller ABI emission-pass start anchor missing")
if source.find(second_start_anchor, second_start + 1) >= 0:
    raise SystemExit("extended caller ABI emission-pass start anchor is not unique")
second_end = source.find(second_end_anchor, second_start)
if second_end < 0:
    raise SystemExit("extended caller ABI emission-pass end anchor missing")
...
second_replacement = r'''        {
            for (argument_index = 0U; argument_index < argument_count; ++argument_index) {
                ...
                if (location->value.kind == MINIC_RISCV64_ABI_VALUE_INDIRECT) {
                    ...
                    if (location->stack_slot_count == 1U &&
...
print("MATERIALIZED rv64-caller-abi-location-linux")
