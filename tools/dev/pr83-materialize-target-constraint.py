...
PATCH = Path("tools/dev/pr83-rv64-inline-asm-target-constraint.patch")
CODEGEN = Path("src/target/riscv64/codegen_inline_asm.c")


def normalize_hunks(path: Path) -> None:
    ...
    for (operand_index = 0U; operand_index < operand_count; ++operand_index) {
        ...
        while (candidate_index < MINIC_RISCV64_INLINE_ASM_MAX_OPERANDS &&
               inline_asm_clobbers_register(
                   inline_asm, minic_riscv64_inline_asm_registers[candidate_index])) {
            candidate_index += 1U;
        }
...
''',
        "input validation and register reservation",
    )
    text = replace_once(
        text,
...
''',
        '''    if (!emit_template(
            file, program, inline_asm, statement->inline_asm_id, operand_registers)) {
''',
        "emit template call",
    )
    text = replace_once(
...
''',
        "writeback register mapping",
    )
    CODEGEN.write_text(text)


normalize_hunks(PATCH)
subprocess.run(
    ["git", "apply", "--check", "--exclude=src/target/riscv64/codegen_inline_asm.c", str(PATCH)],
    check=True,
)
subprocess.run(
    ["git", "apply", "--exclude=src/target/riscv64/codegen_inline_asm.c", str(PATCH)],
    check=True,
)
transform_codegen()
