...
foundation_head=b47e67c45405c0e71dd0ce9a6e01aef68f65f718
...
cp tools/dev/materialize-record-datalayout-query-v1.py /tmp/record-query.py
cp tools/dev/run-materialize-record-datalayout-query-v1.py /tmp/record-query-runner.py
cp tools/dev/materialize-record-datalayout-cache-removal.py /tmp/record-cache-removal.py

# Build the exact formal product patch for this slice before leaving the feature head.
...
git diff --check
git diff --binary "$foundation_head" -- \
  src/frontend/ast.c \
  src/frontend/ast.h \
...
# Preserve the already-proven discovery adapters used by the previous Linux gate.
...
# Rebuild the authoritative proven Linux semantic workspace.
...
# Reapply the formal ABI seam while retaining discovery-only callee/caller shapes.
...
# Reapply the already-proven FunctionLayout/local placement ownership bridge.
...
if grep -R -n --include='*.c' --include='*.h' 'local->storage_offset\|object->storage_offset' src/target/riscv64; then
  echo 'hybrid local storage_offset consumer remains' >&2
  exit 1
fi

# Finally overlay this record DataLayout ownership slice. Conflicts are evidence,
# not permission to silently choose a side.
set +e
git apply --3way --index /tmp/record-datalayout.patch
apply_status=$?
set -e
if test "$apply_status" -ne 0; then
  echo 'RECORD_DATALAYOUT_3WAY_CONFLICTS_BEGIN'
  git status --short
  for path in $(git diff --name-only --diff-filter=U); do
    echo "--- conflict:$path ---"
    grep -n -B 8 -A 24 -E '^(<<<<<<<|=======|>>>>>>>)' "$path" || true
  done
  echo 'RECORD_DATALAYOUT_3WAY_CONFLICTS_END'
  exit 1
fi
CLANG_FORMAT=clang-format-18 bash tools/maintenance/run-format.sh write
git add -A
git diff --cached --check
if grep -R -n --include='*.c' --include='*.h' 'field->storage_offset\|field->bit_offset\|record->storage_size\|record->alignment' src; then
  echo 'record layout cache remains in Linux hybrid' >&2
  exit 1
fi

echo 'RECORD_DATALAYOUT_HYBRID=1'
make -j4 MODE=release BUILD_DIR=build/record-datalayout-hybrid-linux
...
test "$lines" -eq 90928
set +e
build/record-datalayout-hybrid-linux/bin/minic -S \
