...
# Older convergence compilers had a single monolithic relocation guard here.
# The generalized integer-address relocation owner splits slot classification
# before validation, so the old diagnostic insertion point intentionally no
# longer exists. Retain traversal diagnostics and leave that product validation
# path untouched when the anchor is absent.
...
if count == 1:
    ...
    raise SystemExit(f"relocation anchor count={count}")
