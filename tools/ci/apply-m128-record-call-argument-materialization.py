...
if marker not in text:
    ...
    old = '''    argument_status =
        lower_record_value_address(context, argument_id, parameter_type, &source_pointer);
'''
    new = '''    /* M128_RECORD_CALL_ARGUMENT_MATERIALIZATION: a by-value record
    ...
    argument_status =
        lower_record_materialized_address(context, argument_id, parameter_type, &source_pointer);
'''
    if segment.count(old) != 1:
        raise SystemExit('record call argument address-only seam changed')
