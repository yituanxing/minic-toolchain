...
text = p.read_text()
old = "    block->instruction_count -= 7U;\n"
new = '''    /* Core verification requires every global instruction/value to be owned by
    ...
    }
    {
        ...
        if (getenv("MINIC_LOCAL_FACT_TRACE") != NULL) {
            (void)fprintf(stderr,
                          "RK_EARLY_GLOBAL_ROLLBACK function=%s removed_insts=7 removed_values=%zu\\n",
...
'''
if text.count(old) != 1:
    raise SystemExit(f"expected one block-tail rewind, found {text.count(old)}")
