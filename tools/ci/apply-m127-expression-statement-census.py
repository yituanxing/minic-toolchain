    ...
    if (context->source_function != NULL && context->source_function->name != NULL &&
        ...
        if (expression->kind == MINIC_EXPRESSION_STATEMENT &&
            ...
            if (statement_result != NULL) {
                statement_result_kind = (int)statement_result->kind;
            }
        }
        (void)fprintf(stderr,
                      "CORE_M129_EXPR_STMT function=%s span=%zu:%zu kind=%d value_category=%d "
                      "unary_operator=%d binary_operator=%d statement_result_kind=%d\\n",
...
condition_new = '''    status = lower_expression(context, expression_id, &condition);
    if (status != MINIC_CORE_LOWER_OK) {
        ...
        (void)fprintf(stderr,
                      ...
                      "unary_operator=%d binary_operator=%d left_kind=%d right_kind=%d\\n",
...
print('M127/M129 nested expression frontier census staged')
