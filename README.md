...
- integer arithmetic, comparisons, left/right shifts, bitwise AND/XOR, conditions, `if`, `while`, normalized conditioned and empty-condition `for` loops, innermost-loop `break`, ordinary and compound-XOR assignments, general expression statements with discarded values, prefix increment, pointers, fixed and recursive arrays, repeatable postfix subscripting on suitable expression results, pointer arithmetic, comma-separated local declarations, and const-qualified local initialization;
