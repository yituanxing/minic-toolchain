...
- 整数表达式、比较、条件、`if`、`while`、规范化的受限 `for` 循环、赋值、前置递增、指针、固定数组、指针算术和逗号局部声明；
- 函数原型、合法前向调用、直接与嵌套调用、递归、互递归，以及 0～8 个整数寄存器参数；
- `void`、`const`、命名结构、递归数组 typedef、静态只读全局数组和内部函数；
- RV64 对象布局、调用安全栈帧、signed/unsigned 加载与运算、汇编发射和内部符号可见性；
- Debug、Release `-Werror`、ASan/UBSan、RV64/QEMU 和 GCC/MiniC 双轨差分门禁。

当前有 23 个可执行 C 程序永久运行两条流水线，并比较退出码、标准输出和标准错误。矩阵包含循环变量、循环体执行次数的隔离测试，以及高位 unsigned 比较、除法和取余测试。

## 第一个外部真实项目

第一个固定上游驱动项目是 `kokke/tiny-AES-c`，当前配置为 AES-128 ECB。CI 会按固定 Git Blob 校验上游源码，不允许为 MiniC 修改上游文件。

编译器已经越过上游的声明、typedef 数组、静态查找表、内部函数、`void` 函数定义、unsigned 声明列表和 `KeyExpansion` 的第一条循环。当前精确前沿是首个对 `RoundKey` 和 `Key` 指针参数进行下标访问的赋值。
