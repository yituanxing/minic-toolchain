...
- 带左值/右值区分，并具有明确 signed/unsigned 整数身份的类型化表达式；
- 词法块作用域，以及由 Program 稳定持有的局部对象；
- 整数表达式、比较、条件、循环、赋值、指针、固定数组、指针算术和逗号局部声明；
...
编译器已经越过上游的声明、typedef 数组、静态查找表、内部函数、`void` 函数定义，以及 `KeyExpansion` 中的逗号声明 `unsigned i, j, k`。当前精确前沿位于该函数的第一条 `for` 语句。
