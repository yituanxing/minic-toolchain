...
- RV64 对象布局、调用安全栈帧、signed/unsigned 加载与运算、数组/指针下标共享缩放地址计算、汇编发射和内部符号可见性；
- Debug、Release `-Werror`、ASan/UBSan、RV64/QEMU 和 GCC/MiniC 双轨差分门禁。

当前有 25 个可执行 C 程序永久运行两条流水线，并比较退出码、标准输出和标准错误。矩阵包含循环变量与循环体执行次数的隔离测试、高位 unsigned 比较/除法/取余、指针参数和局部指针下标读写，以及跨函数调用的 const 局部初始化与读取。

## 第一个外部真实项目

第一个固定上游驱动项目是 `kokke/tiny-AES-c`，当前配置为 AES-128 ECB。CI 会按固定 Git Blob 校验上游源码，不允许为 MiniC 修改上游文件。

编译器已经越过上游的声明、typedef 数组、静态查找表、内部函数、`void` 函数定义、unsigned 声明列表、`KeyExpansion` 的第一条循环、指针参数下标和 const 限定局部初始化。当前精确前沿是 `KeyExpansion` 中首次把静态全局 `sbox` 查找表作为表达式引用的位置。
