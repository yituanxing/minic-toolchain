...
- 整数表达式、比较、条件、`if`、`while`、规范化的受限 `for` 循环、赋值、前置递增、指针、固定数组、数组与指针下标、指针算术和逗号局部声明；
...
当前有 24 个可执行 C 程序永久运行两条流水线，并比较退出码、标准输出和标准错误。矩阵包含循环变量与循环体执行次数的隔离测试、高位 unsigned 比较/除法/取余，以及指针参数和局部指针的下标读写。

## 第一个外部真实项目

第一个固定上游驱动项目是 `kokke/tiny-AES-c`，当前配置为 AES-128 ECB。CI 会按固定 Git Blob 校验上游源码，不允许为 MiniC 修改上游文件。

编译器已经越过上游的声明、typedef 数组、静态查找表、内部函数、`void` 函数定义、unsigned 声明列表、`KeyExpansion` 的第一条循环和指针参数下标。当前精确前沿是源码中的 `const uint8_t u8tmp = tempa[0]`；在现有目标 shim 下，它会被预处理为 const 限定的 `int` 局部声明。
