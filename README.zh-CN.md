...
- 整数表达式、比较、按位异或、条件、`if`、`while`、规范化的受限 `for` 循环、赋值、求值后丢弃结果的通用表达式语句、前置递增、指针、固定与递归数组、适用于任意合适表达式结果的可重复后缀下标、指针算术、逗号局部声明和 const 限定局部初始化；
...
- RV64 标量/数组/记录对象布局、共享类型大小/对齐查询、字段偏移与聚合对齐、调用安全栈帧、signed/unsigned 加载与运算、按位异或降低、成员基址加偏移寻址、表达式语句求值与结果丢弃、2 的幂与任意聚合下标跨度缩放、汇编发射和内部符号可见性；
- Debug、Release `-Werror`、ASan/UBSan、RV64/QEMU 和 GCC/MiniC 双轨差分门禁。

当前有 30 个可执行 C 程序永久运行两条流水线，并比较退出码、标准输出和标准错误。矩阵包含循环变量与循环体执行次数的隔离测试、高位 unsigned 比较/除法/取余、指针参数和局部指针下标读写、const 局部初始化与读取、带局部同名遮蔽的静态全局数组查找、signed/unsigned 按位异或、带非零字段偏移的记录指针成员、通用表达式语句，以及覆盖非 2 的幂 12 字节行跨度的多维数组指针下标。

## 第一个外部真实项目

第一个固定上游驱动项目是 `kokke/tiny-AES-c`，当前配置为 AES-128 ECB。CI 会按固定 Git Blob 校验上游源码，不允许为 MiniC 修改上游文件。

编译器已经越过上游的声明、typedef 数组、静态查找表、内部函数、`void` 函数定义、unsigned 声明列表、`KeyExpansion` 的第一条循环、指针参数下标、const 限定局部初始化、静态全局 `sbox` 表的表达式引用、按位异或、typedef 记录字段、记录布局、首次 `ctx->RoundKey` 指针成员访问、独立的 `KeyExpansion(ctx->RoundKey, key);` 表达式语句，以及 `(*state)[i][j]` 的可重复后缀下标。当前精确前沿是 `AddRoundKey` 中的复合异或赋值运算符。
