...
static void minipp_debug_pack_origin(const char *stage,
                                     const MiniPpMacro *macro,
                                     const MiniPpArgList *args,
                                     bool preserve,
                                     FILE *diagnostics) {
    size_t arg_index;
    bool wanted = strncmp(macro->name, "OL_", 3U) == 0 ||
                  strcmp(macro->name, "COUNTER_TWICE") == 0;

    if (!wanted) {
        return;
    }
    fprintf(diagnostics,
            "MINIPP_PACK stage=%s macro=%s preserve=%d count=%zu\n",
    ...
    for (arg_index = 0U; arg_index < args->count; ++arg_index) {
        ...
        size_t i;
        fprintf(diagnostics,
                "MINIPP_PACK_ARG macro=%s idx=%zu lead=%d gen=%d str=%d data=",
                macro->name,
                arg_index,
...
static bool minipp_build_logical_args(MiniPpState *state,
    ...
    minipp_debug_pack_origin("logical",
                              macro,
                              logical_args,
                              preserve_argument_spacing,
                              state->diagnostics);
