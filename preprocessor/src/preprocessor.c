...
static bool minipp_handle_include(MiniPpState *state,
    ...
    if (!(include_next
        ...
        fprintf(state->diagnostics,
                include_next ? "minic-cpp: include-next-not-found:%s\n"
...
int minipp_preprocess_file(const char *input_path,
    ...
    state.system_include_paths = config->system_include_paths;
    state.system_include_path_count = config->system_include_path_count;
