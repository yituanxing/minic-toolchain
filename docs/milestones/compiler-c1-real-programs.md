...
The compiler-first track now compiles complete single- and multi-function algorithm programs. Development is driven by executable behavior rather than isolated syntax acceptance.

编译器优先主线现在可以编译完整的单函数和多函数算法程序。开发以可执行行为为驱动，而不再只验证孤立语法。
...
The function-composition program has been run through both lanes with equal exit status and equal empty stdout/stderr.

函数组合程序已经实际通过两条流水线，退出码一致，stdout/stderr 均为空且一致。
...
- multiple zero-argument `int name(void)` or `int name()` definitions / 多个零参数整型函数定义；
- independent local ranges and call-safe RV64 frames for every function / 每个函数独立的局部范围和调用安全 RV64 栈帧；
- resolved zero-argument direct calls, nested calls, and self-reference syntax / 已解析的零参数直接调用、嵌套调用和自引用语法；
- preservation and restoration of `ra` and `s0` / `ra` 与 `s0` 的保存和恢复。

## C language boundary / C 语言边界

Calls are currently legal when the callee has already been declared by a preceding definition, or when a function references itself. MiniC deliberately rejects an undeclared call instead of accepting an implicit function declaration, because implicit declarations are not valid ISO C11.

当前调用要求被调用函数已经由此前的函数定义完成声明，或者是当前函数自引用。MiniC 会拒绝未声明调用，而不是接受隐式函数声明，因为隐式声明不属于合法 ISO C11。

The next frontend step is explicit zero-argument function prototypes such as:
...
- function prototypes and declared-before-defined functions / 函数原型及先声明后定义；
...
This milestone does not claim readiness for Lua, SQLite, musl, or Linux. Those workloads remain later drivers after functions, parameters, pointers, arrays, and object storage are established.

本里程碑不代表已经可以编译 Lua、SQLite、musl 或 Linux。这些负载要在函数、参数、指针、数组和对象存储建立后再作为后续驱动目标。
...
1. explicit zero-argument function prototypes;
2. one to eight integer parameters with RV64 register arguments;
